 ...
 const handleDateClick = async (info: { dateStr: string }) => {
   ...
   try {
     if (selectedDates.includes(info.dateStr)) {
       // Remove date
       await Util.request(`/calendar/${info.dateStr}`, { method: "DELETE" });
       setSelectedDates((prevDates) =>
         prevDates.filter((date) => date !== info.dateStr)
       );
     } else {
       // Add date
       await Util.post(`/calendar/${info.dateStr}`, {});
       setSelectedDates((prevDates) => [...prevDates, info.dateStr]);
     }
   } catch (error) {
